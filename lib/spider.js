...
let login = co.wrap(function*(conf) {
    ...
    let cookie = jar.getCookies(baseUrl);
    const csrfToken = cookie.find(element => {
        return element.key === 'csrftoken';
    });
    assert.notEqual(csrfToken, undefined, 'network error: cannot get csrftoken');
    ...
    cookie = jar.getCookies(baseUrl);
    assert.notEqual(
        cookie.find(element => element.key === 'LEETCODE_SESSION'),
        undefined,
...
let parallelFetch = co.wrap(function*(acLists) {
    ...
    if (acLists && acLists.length > 0) {
        ...
        let acListPromises = acLists.map(acProblem => {
            ...
            languageCodeMap._acceptance =
                ((acProblem['stat']['total_acs'] / acProblem['stat']['total_submitted']) * 100).toFixed(2) + '%';
...
let fetchAndWrite = function*(problemInfo, languageToFetch, languageCodeMap) {
    ...
    yield fetchQuestion(problemInfo, languageCodeMap).catch(err => {
        throw err;
    });
    yield fileUtils.writeToFile(languageCodeMap, solutionsDirPath).catch(err => {
        throw err;
    });
...
let fetchACSolutionOfProblem = co.wrap(function*(problemInfo, languageToFetch, page, languageCodeMap) {
    ...
    submissionsJson.forEach(e => {
        ...
        if (~languageToFetch.indexOf(language)) {
            ...
            acSolutionPromise.push(
                fetchSolutionsOfUrl(codeUrl, 0)
                    .then(codeObj => {
                    ...
                    .catch(err => {
                        logger.error(
                            'Fetching the ' +
                                language +
                                ' code of ' +
                                problemInfo.stat['question__title_slug'] +
                                ' failed',
                        );
                        logger.error(err.stack);
                    }),
...
let fetchQuestion = co.wrap(function*(problemInfo, languageCodeMap) {
    ...
    try {
        responseAndBody = yield request({
            url: '/graphql',
            method: 'POST',
            origin: baseUrl,
            referer: `${baseUrl}/problems/two-sum/`,
            'x-newrelic-id': 'UAQDVFVRGwEAXVlbBAg=',
            'x-csrftoken': csrfToken,
            json: {
                operationName: 'questionData',
                variables: { titleSlug: problemInfo.stat['question__title_slug'] },
                query:
    ...
    languageCodeMap['_question'] = responseAndBody[1].data.question.content;
});
